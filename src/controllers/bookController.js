...
const getBooksInYear= async function (req, res) {
    ...
    let BooksInYear = await bookModel.find({year:body})
    res.send({msg:BooksInYear})
}

const getParticularBooks = async function(req,res){
    let bodys = req.body
    let ParticularBooks = await bookModel.find({$or:[{bookName:bodys.bookName},{year:bodys.year},{tags:bodys.tags},{totalPages:bodys.totalPages},{stockAvailable:bodys.stockAvailable},{authorName:bodys.authorName}]})
    res.send({msg:ParticularBooks})
}


const getXINRBooks= async function (req, res){
    let booksByINR= await bookModel.find({$or: [{'prices.indianPrice':{$eq: 900}}, {'prices.indianPrice':{$eq: 500}}, {'prices.indianPrice':{$eq: 800}}]})
